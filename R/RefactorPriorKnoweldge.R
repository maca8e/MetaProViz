...
#' #' if a single resource is provided with a vector of column names in \code{SettingsInfo}, compares columns within that resource.
#' #'
#' #' In the multi-resource mode, each element in \code{pk_list} represents a PK resource (either as a data frame or a recognized resource name)
#' #' from which a set of features is extracted. A binary summary table is then constructed and used to create an UpSet plot.
#' #'
#' #' In the within-resource mode, a single data frame is provided (with \code{pk_list} containing one element) and its \code{SettingsInfo} entry
#' #' is a vector of column names to compare (e.g., binary indicators for different annotations). In this case, the function expects the data frame
#' #' to have a grouping column named \code{"Class"} (or, alternatively, a column specified via the \code{class_col} attribute in \code{SettingsInfo})
#' #' that is used for grouping in the UpSet plot.
#' #'
#' #' @param pk_list A named list where each element corresponds to a prior knowledge (PK) resource. Each element can be:
#' #'        \itemize{
#' #'          \item A data frame containing gene/metabolite identifiers (and additional columns for within-resource comparison),
#' #'          \item A character string indicating the resource name. Recognized names include (but are not limited to): \code{"Hallmarks"},
#' #'                \code{"Gaude"}, \code{"MetalinksDB"}, and \code{"RAMP"} (or \code{"LoadRAMP"}). In the latter case, the function
...
#' #'        }
#' #'
#' #' @param SettingsInfo A named list (with names matching those in \code{pk_list}) where each element is either a character string or a
#' #'        character vector indicating the column name(s) to extract features. For multiple-resource comparisons, these refer to the columns
#' #'        containing feature identifiers. For within-resource comparisons, the vector should list the columns to compare (e.g., \code{c("CHEBI", "HMDB", "LIMID")}).
#' #'        In within-resource mode, the input data frame is expected to contain a column named \code{"Class"} (or a grouping column specified via the
#' #'        \code{class_col} attribute). \emph{If no grouping column is found, a default grouping column named \code{"Group"} (with all rows assigned the same value) is created.}
...
#' #'
#' #' # Using automatic data loading for multiple resources.
#' #' pk_list <- list(Hallmarks = "Hallmarks", Gaude = "Gaude",
#' #'                 MetalinksDB = "MetalinksDB", RAMP = "LoadRAMP")
#' #' res <- ComparePK(pk_list = pk_list)
#' #'
#' #' # Filtering to include only gene features:
#' #' res_genes <- ComparePK(pk_list = pk_list, filter_by = "gene")
#' #'
#' #' ## Example 2: Within-Resource Comparison (Comparing Columns Within a Single Data Frame)
#' #'
#' #' # Assume FeatureMetadata_Biocrates is a data frame with columns: "TrivialName", "CHEBI", "HMDB", "LIMID", and "Class".
#' #' # Here the "Class" column is used as the grouping variable in the UpSet plot.
#' #' pk_list_single <- list(Biocft = FeatureMetadata_Biocrates)
#' #' SettingsInfo_single <- list(Biocft = c("CHEBI", "HMDB", "LIMID"))
#' #'
#' #' res_single <- ComparePK(pk_list = pk_list_single, SettingsInfo = SettingsInfo_single,
#' #'                           plot_title = "Overlap of BioCrates Columns")
#' #'
#' #' ## Example 3: Custom Data Frames with Custom Column Names
#' #'
...
#' #'                            gene_symbol = c("GENE1", "GENE4"), stringsAsFactors = FALSE)
#' #' ramp_df <- data.frame(class_source_id = c("HMDB0001", "HMDB0003"), stringsAsFactors = FALSE)
#' #' pk_list <- list(Hallmarks = hallmarks_df, Gaude = gaude_df,
#' #'                 MetalinksDB = metalinks_df, RAMP = ramp_df)
#' #' SettingsInfo <- list(Hallmarks = "feature", Gaude = "feature",
#' #'                      MetalinksDB = c("hmdb", "gene_symbol"), RAMP = "class_source_id")
#' #' res <- ComparePK(pk_list = pk_list, SettingsInfo = SettingsInfo, filter_by = "metabolite")
#' #'
#' #' @importFrom dplyr mutate select
#' #' @importFrom utils write.csv
#' #' @export
#' ComparePK <- function(pk_list, SettingsInfo = NULL,
#'                       filter_by = c("both", "gene", "metabolite"),
#'                       plot_title = "Overlap of Prior Knowledge Resources",
#'                       palette_type = "polychrome",
#'                       output_file = NULL) {
...
#'   filter_by <- match.arg(filter_by)
#'
#'   # Validate pk_list input
#'   if (!is.list(pk_list) || length(pk_list) < 1) {
#'     stop("pk_list must be a non-empty list.")
#'   }
#'   if (is.null(names(pk_list)) || any(names(pk_list) == "")) {
#'     stop("pk_list must be a named list with resource names.")
#'   }
#'
#'   # Define resource lookup table with information on how to retrieve and transform each resource.
#'   resource_definitions <- list(
...
#'   )
#'
#'   # Preprocess pk_list: auto‑load resources if they are provided as strings.
#'   for (res in names(pk_list)) {
#'     if (!inherits(pk_list[[res]], "data.frame") && is.character(pk_list[[res]])) {
#'       resource_id <- tolower(pk_list[[res]])
#'       if (resource_id %in% names(resource_definitions)) {
#'         res_def <- resource_definitions[[resource_id]]
#'         if (exists(res_def$var, envir = .GlobalEnv)) {
#'           resource_object <- get(res_def$var, envir = .GlobalEnv)
...
#'           resource_object <- res_def$load_fun()
#'         }
#'         pk_list[[res]] <- res_def$transform_fun(resource_object)
#'         if (is.null(SettingsInfo[[res]])) {
#'           SettingsInfo[[res]] <- res_def$default_col
#'         }
#'       }
...
#'   # Determine if we are in within-resource mode.
#'   # If only one resource is provided and its SettingsInfo entry has >1 column, assume within-resource comparison.
#'   single_resource <- (length(pk_list) == 1)
#'   within_resource_mode <- FALSE
#'   if (single_resource) {
#'     resource_name <- names(pk_list)[1]
#'     if (!is.null(SettingsInfo[[resource_name]]) &&
#'         length(SettingsInfo[[resource_name]]) > 1) {
#'       within_resource_mode <- TRUE
#'     }
...
#'     # ===== Within-Resource Comparison Mode =====
#'     # Retrieve the single data frame.
#'     resource_data <- pk_list[[resource_name]]
#'
#'     # Identify the intersection columns based on SettingsInfo.
#'     intersect_cols <- SettingsInfo[[resource_name]]
#'     missing_cols <- setdiff(intersect_cols, colnames(resource_data))
...
#'   } else {
#'     # ===== Multi-Resource Comparison Mode =====
#'     # Process each resource in pk_list.
#'     for (res in names(pk_list)) {
#'       resource_val <- pk_list[[res]]
#'       if (!inherits(resource_val, "data.frame")) {
#'         if (!is.character(resource_val)) {
#'           stop("Each element in pk_list must be either a data frame or a character string indicating a resource name.")
#'         }
#'         resource_id <- tolower(resource_val)
#'         if (resource_id %in% c("loadramp")) {
#'           resource_id <- "ramp"
...
#' ##########################################################################################
#' ### Compare Prior Knowledge Resources and Generate an UpSet Plot                       ###
#' ##########################################################################################
#'
#' #' Compare Prior Knowledge Resources,  and Generate an UpSet Plot
#' #'
#' #' Handles metabolites (prefixed by HMDB), genes (no prefix), or both simultaneously (default).
#' #' In the simplest use case, user can pass PK names such as 'Hallmarks', 'Gaude', 'MetalinksDB' or 'RAMP'
#' #' to compare. Alternatively, the user could pass a custom df, or multiple dfs, to compare against, in
#' #' which case they should also specify the column names to be used for the gene/metabolite comparison.
#' #'
#' #' @param pk_list A named list where each element corresponds to a prior knowledge (PK) resource.
#' #'        Each element can either be:
#' #'        \itemize{
#' #'          \item A data frame containing gene/metabolite identifiers, or
#' #'          \item A character string indicating the resource name.
#' #'                Recognized names include (but are not limited to): "Hallmarks", "Gaude",
#' #'                "MetalinksDB", "RAMP" (or "LoadRAMP"). For each, the function will first
#' #'                check if a corresponding global variable exists, and if not, will load it automatically.
#' #'        }
#' #'
#' #' @param SettingsInfo \emph{Optional:} A named list where each element is either a character string or a character vector
#' #'        indicating the column name(s) in the corresponding data frame that contain the gene/metabolite identifiers.
#' #'        If omitted for a known resource, defaults are assigned.
#' #'
#' #' @param filter_by Character. Optional filter for the resulting features.
#' #'        Options are: "both" (default), "gene", or "metabolite". If "gene", only gene features are retained;
#' #'        if "metabolite", only metabolite features (i.e. those starting with "HMDB") are retained.
#' #'
#' #' @param plot_title Character. Title for the UpSet plot. \strong{Default =} "Overlap of Prior Knowledge Resources"
#' #' @param palette_type Character. Color palette to be used in the plot. \strong{Default =} "polychrome"
#' #' @param output_file \emph{Optional:} File path to save the generated plot.
#' #'
#' #' @return A list containing two elements:
#' #' \item{summary_table}{A data frame containing a binary matrix of feature presence/absence across the PK resources,
#' #'                      along with a feature type classification (optionally filtered by gene/metabolite).}
#' #' \item{upset_plot}{The generated UpSet plot object.}
#' #'
#' #' @examples
#' #' # Example using automatic data loading (no filtering):
#' #' pk_list <- list(Hallmarks = "Hallmarks", Gaude = "Gaude",
#' #'                 MetalinksDB = "MetalinksDB", RAMP = "LoadRAMP")
#' #' res <- MetaProViz::ComparePK(pk_list = pk_list)
#' #'
#' #' # Example filtering to include only gene features:
#' #' res_genes <- MetaProViz::ComparePK(pk_list = pk_list, filter_by = "gene")
#' #'
#' #' # Example with preloaded data frames and custom column names:
#' #' hallmarks_df <- data.frame(feature = c("HMDB0001", "GENE1", "GENE2"), stringsAsFactors = FALSE)
#' #' gaude_df <- data.frame(feature = c("GENE2", "GENE3"), stringsAsFactors = FALSE)
...
#' #' SettingsInfo <- list(Hallmarks = "feature", Gaude = "feature",
#' #'                      MetalinksDB = c("hmdb", "gene_symbol"), RAMP = "class_source_id")
#' #' res <- MetaProViz::ComparePK(pk_list = pk_list, SettingsInfo = SettingsInfo, filter_by = "metabolite")
#' #'
#' #' @keywords Compare Prior Knowledge, UpSet Plot, Feature Overlap, Extendable Resources, Filtering
#' #'
#' #' @importFrom dplyr mutate select
#' #' @importFrom utils write.csv
#' #' @export
...
#'       load_fun = MetaProViz::LoadRAMP,
#'       transform_fun = function(x) {
#'         # For RAMP, the global variable ChemicalClass_MetabSet is assumed to be a data frame on its own.
#'         x
#'       },
#'       default_col = "class_source_id"
#'     )
...
#'     resource_val <- pk_list[[res]]
#'     if (!inherits(resource_val, "data.frame")) {
#'       # Expect a character string as an identifier.
#'       if (!is.character(resource_val)) {
#'         stop("Each element in pk_list must be either a data frame or a character string indicating a resource name.")
#'       }
#'       # Normalize the identifier and handle any synonyms.
#'       resource_id <- tolower(resource_val)
#'       if (resource_id %in% c("loadramp")) {
#'         resource_id <- "ramp"
#'       }
...
#'              ". Please provide a data frame or a valid resource name.")
#'       }
#'       # Retrieve the resource: first try to get the variable from the global environment.
#'       res_def <- resource_definitions[[resource_id]]
#'       if (exists(res_def$var, envir = .GlobalEnv)) {
#'         resource_object <- get(res_def$var, envir = .GlobalEnv)
#'       } else {
#'         resource_object <- res_def$load_fun()
#'       }
#'       # Transform the resource into the desired data frame.
#'       pk_list[[res]] <- res_def$transform_fun(resource_object)
#'       # Set default SettingsInfo if not provided.
#'       if (is.null(SettingsInfo[[res]])) {
#'         SettingsInfo[[res]] <- res_def$default_col
#'       }
#'     } else {
#'       # If a data frame is already provided and no SettingsInfo entry exists,
...
#'   }
#'
#'   # Validate that SettingsInfo is a list and that names match.
#'   if (!is.list(SettingsInfo)) {
#'     stop("SettingsInfo must be a list.")
#'   }
#'   if (!setequal(names(pk_list), names(SettingsInfo))) {
#'     stop("The names of pk_list and SettingsInfo must match.")
#'   }
#'
#'   # Extract features from each resource based on SettingsInfo.
#'   resource_features <- list()
...
#'   df_binary$None <- as.integer(rowSums(df_binary[, resource_cols, drop = FALSE]) == 0)
#'
#'   # Optionally filter the summary table by gene or metabolite.
#'   if (filter_by == "gene") {
#'     df_binary <- subset(df_binary, Type == "gene")
#'   } else if (filter_by == "metabolite") {
#'     df_binary <- subset(df_binary, Type == "metabolite (HMDB)")
#'   }
#'
#'   # Generate the UpSet plot using the helper function.
#'   upset_plot <- MetaProViz:::VizUpset(
#'     df = df_binary,
#'     class_col = "Type",
#'     intersect_cols = resource_cols,
