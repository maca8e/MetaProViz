...
\
Yet, many metabolomics studies do not report multiple IDs, but rather one ID per measured peak. In some cases the chosen ID is correct in representing the degree of ambiguity in the detection, e.g. "Alanine-Zwitterion", whilst in other cases a specific ID, which has not been detected in this specificity, e.g. "L-Alanine". In both cases this can lead to missed maps to prior knowledge and hence a loss of information as discussed above.\
...
Specifically in metabolite PK, the many different PK databases and resources pose several issues. Indeed, the metabolite identifiers (e.g. KEGG, HMDB, PubChem, etc.) are not standardized across databases, and the same metabolite can have multiple identifiers in different databases. This is known as the many-to-many mapping problem. Moreover, metabolic pathways that are the basis of the PK databases also includes co-factors such as ions or other small molecules that are not only part of most reactions, but can also not be detected in experimentally acquired data (e.g. H2O, CO2, etc).\
To address these issues and  provide a standardized way to access and integrate metabolite PK, we provide access to several prior knowledge resources and removed molecules such as water. We terme this collection of metabolite sets MetSigDB (Metabolite signature database).
...
The function `Make_GeneMetabSet` can be used to convert gene names to metabolite names by using a PK network of metabolic reactions calls CosmosR [@Dugourd2021]. This function is useful if you want to perform pathway enrichment analysis on available gene-sets such as the Hallmarks gene-sets from MSigDB [@Castanza2022]. Moreover, it enables you to perform combined pathway enrichment analysis on metabolite-gene sets, if you have other data types such as proteomics measuring the enzymes expression.\
...
We have now loaded a number of different PK sources (details above) and aim to compare the overlap in coverage between these PK sources to understand if there are certain genes or metabolites covered by one PK resource but not the others. Here we will focus on comparing the resources containing gene-metabolite sets (Hallmarks, Gaude, MetalinksDB).\
`MetaProViz:::VizUpset()` helper function, which will generate an Upset plot showing the overlap of coverage (similar to how a Venn diagram works, but better able to visualize multiple combinations clearly).
```{r}
...
MetaProViz:::VizUpset(df = df_upset_pk_comparison,
                          class_col = "Type",
...
Refactored version is below - keeping here to show equivalency of outputs
...
res <- MetaProViz:::ComparePK(InputData = pk_list)
res
```
...
pk_list <- list(Hallmarks = "Hallmarks", Gaude = "Gaude",
                MetalinksDB = "MetalinksDB", RAMP = "RAMP")
res <- MetaProViz:::ComparePK(InputData = pk_list, filter_by='gene')
res
```
Without RAMP
```{r}
...
res
```
Without RAMP and with filtering
```{r}
pk_list <- list(Hallmarks = "Hallmarks", Gaude = "Gaude",
                MetalinksDB = "MetalinksDB")
res <- MetaProViz:::ComparePK(InputData = pk_list, filter_by='metabolite')
res
```
...
res <- MetaProViz:::ComparePK(InputData = pk_list, SettingsInfo = SettingsInfo)
res

pk_list2 <- list(Hallmarks = hallmarks_df, Gaude = gaude_df,
                MetalinksDB = "MetalinksDB")
SettingsInfo <- list(Hallmarks = "feature", Gaude = "feature",
                     MetalinksDB = c("hmdb", "gene_symbol"))
res <- MetaProViz:::ComparePK(InputData = pk_list2, SettingsInfo = SettingsInfo)
res
```

\
The UpsetPlot shows that MetalinksDB has unique genes and metabolites not present in the other resources. This is to be expected, since MetalinksDB focuses on receptors, transporters and metabolic enzymes, whilst Gaude and Hallmakrs focus on pathways.\
...
The experimental data in this example will be the data loaded in section 1 (Loading the example data). Whichever resource the experimental data resembles, we recommend to follow this overall approach:\
\
1.  Determine the metabolite identifiers from the experimental data and inspect their coverage.\
2.  Select the metabolite identifiers from the experimental data, which can be used to link to the PK of interest.\
3.  Connect metabolite identifiers to the PK and assess overlap.\
\
Here one can select any combination of experimental data and PK. This selection should be based on the experimental data at hand and the biological research question.\
...
In many experimentally aquired data, there can be different metabolite IDs used to describe the metabolite. In the Biocrates data the available metabolite identifiers are mostly CHEBI, HMDB, and LIMID that could be linked to PK resources. Now we select these metabolite identifier columns to count the coverage and look at the combinations of the coverage grouped by the class of metabolite. This helps us to understand what to expect, when linking the data to prior knowledge.\

```{r}
pk_list <- list(biocrates_triv = FeatureMetadata_Biocrates,
                biocrates_cheb = FeatureMetadata_Biocrates)
SettingsInfo <- list(biocrates_triv = c("TrivialName"),
                     biocrates_cheb = c("CHEBI"))
res <- MetaProViz:::ComparePK(InputData = pk_list, SettingsInfo = SettingsInfo)
res
```
...
res_single <- ComparePK(InputData = pk_list_single, SettingsInfo = SettingsInfo_single,
                           plot_title = "Overlap of BioCrates Columns")
res_single
```

```{r}
pk_list_single <- list(MetalinksDB = "MetalinksDB")
SettingsInfo_single <- list(MetalinksDB = c("hmdb", "gene_symbol","transport_direction"))
res_single <- ComparePK(InputData = pk_list_single, SettingsInfo = SettingsInfo_single,
                           plot_title = "Overlap of MetalinksDB Columns")
res_single
```
...
MetaProViz:::VizUpset(df = df_upset_biocft,
                          class_col = "Class",
...
-   Less than half the metabolites had ***CHEBI+HMDB+LIMID (n=440)***, which isn't great but probably higher than we have expected.\
...
So actually, relying upon only LIMID means we miss 151 biocrates metabolites (89 with only HMDB/ChEBI + 22 with only CHEBI + 8 with only HMDB and 32 with no LIMID/CHEBI/HMDB identifiers).
...
We have inspected the experimental coverage of the metabolite IDs from the Biocrates kit. Next, we have to choose which metabolite identifier to use to link to the PK of choice. Often this choice is dictated by the prior knowledge resource as most of them use a specific identifier.\
Here we will use MetalinksDB, which uses HMDB IDs as metabolite identifiers, hence it is best to use the HMDB IDs to link the Biocrates features to the resource. Noteworthy, in some cases the Biocrates metabolites have multiple HMDB IDs listed per metabolite:\
```{r, fig.width=8, fig.height=6}
...
We discuss the pros and cons of this later in section: [Bonus: Are multiple metabolite IDs helpful or a hindrance?].
...
-   `InputData_Matched_NA_and_duplicates`: the same kind of summary table but with all NA and duplicates included. Helpful for plotting.

-   `InputData_Matched_only_NA`: the same kind of summary table but with only rows where the InputID was NA. Helpful for inspecting which rows were NA.
...
```
\
Here we can immediately see that despite the ID translation, some of the translations between the KEGG MetaboliteID and the HMDB or PubChem IDs have failed, resulting in NA values. To get a better understanding of the combinations of these, let's visualise the translation for each of the ID types.
\
...
```
\
We can also note from the previous table that it becomes apparent that the translation of IDs is not a one-to-one mapping, but rather a one-to-many mapping. In fact it is very common that an ID from one format will have a genuine one-to-many relationship with the other format (e.g. one KEGG ID maps to multiple HMDB IDs) or even a many-to-many relationship, where some of the IDs from the new format link back to multiple IDs in the original format (e.g. two different KEGG IDs map to multiple HMDS IDs, some of which are shared between them).\
...
```
\
In fact, to perform enrichment analysis we need a column source (=e.g. term) and we would want to keep the metabolite IDs across pathways consistent, avoid ambiguous mapping as much as possible (many-to-many mapping), and have this metabolite ID selection guided by any IDs we might have available to us in our measured input data (Fig. 3). This is crucial to circumvent inflation or deflation of metabolite-sets, which in turn will affect the enrichment analysis results. Hence, we can use measured metabolite IDs to guide the selection of PK IDs.
